...
func TestHandlerInsertOnDuplicateKeyUpdate(t *testing.T) {
	svc := newMockService(t)
	t.Cleanup(svc.Close)
	...
	res, err := h.HandleQuery(`INSERT INTO people VALUES ('{"id":"1"}') ON DUPLICATE KEY UPDATE name = 'Jane'`)
	if err != nil {
		t.Fatalf("unexpected upsert error: %v", err)
	}
	if res.AffectedRows != 1 {
		t.Fatalf("expected 1 affected row on duplicate key update, got %d", res.AffectedRows)
	}
